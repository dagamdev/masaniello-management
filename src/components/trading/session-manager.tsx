...
export function SessionManager({ lang, sessionId }: SessionManagerProps) {
  ...
  const { sessions, createSession, deleteSession, updateSessionName, setActiveSession } = useTradingStore()
  ...
  const confirmCreateSession = () => {
    ...
    const newSession = useTradingStore.getState().sessions.at(-1)
    if (newSession) {
  ...
  const confirmDeleteSession = () => {
    ...
    if (wasActive) {
      const remainingSessions = useTradingStore.getState().sessions
